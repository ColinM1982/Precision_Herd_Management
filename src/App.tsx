...
import type { Animal, Farm, RegistryProfile } from './types/database'
...
const nav = [
  ...
  ['/breeding', 'Reproduction', Activity], ['/litters', 'Litters', Baby], ['/outside-herd', 'Archived Animals/Litters', Archive], ['/health', 'Health', HeartPulse], ['/settings', 'Settings', Settings]
] as const
...
function Animals({ farm }: { farm: Farm }) {
  ...
  return <><PageHead eyebrow="ACTIVE SWINE HERD RECORDS" title="Herd Animals" action={<button className="button primary" onClick={()=>setShow(true)}>+ Add animal</button>}/><p className="lead">Sold, culled, deceased, and archived animals are retained under Archived Animals/Litters.</p>{items.length ? <div className="card-grid">{items.map(a=>{const female=['sow','gilt'].includes(a.sex);return <Link className="animal-card" to={`/animals/${a.id}`} key={a.id}><div className="animal-card-head"><div className="avatar"><PiggyBank/></div><div><span className="pill">{a.status}</span><h3>{a.call_name}</h3><p>{[a.breed,a.sex,a.primary_id].filter(Boolean).join(' • ')}</p>{a.registered_name&&<small>{a.registered_name}</small>}</div></div><div className="animal-card-metrics"><span><small>Age</small><strong>{formatAge(a.birth_date)}</strong></span>{female&&<><span><small>Parity</small><strong>{parity[a.id]||0} {parity[a.id]===1?'litter':'litters'}</strong></span><span><small>Reproductive status</small><strong>{reproductiveStatusLabel(a.reproductive_status)}</strong>{a.reproductive_status==='bred'&&a.reproductive_due_date&&<em>Due {formatShortDate(a.reproductive_due_date)}</em>}</span></>}</div></Link>})}</div>:<Empty icon={<PiggyBank/>} title="No active herd animals" text="Add your first animal or restore one from Archived Animals/Litters."/>}{show&&<AnimalModal farm={farm} close={()=>setShow(false)} saved={()=>{setShow(false);load()}}/>}</>
}
