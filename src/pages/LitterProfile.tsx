...
import { Archive, ArrowLeft, Download, ExternalLink, Plus, Save, ShoppingCart, Trash2, UserRoundPlus, X } from 'lucide-react'
...
export default function LitterProfile({farm}:{farm:Farm}) {
  ...
  const totalEarned=pigs.filter(pig=>pig.status==='sold').reduce((total,pig)=>total+Number(pig.sale_price||0),0)
  const completedStatuses=['sold','culled','deceased','died','archived','stillborn','mummified']
  const canArchive=pigs.length>0&&pigs.every(pig=>!!pig.herd_animal_id||completedStatuses.includes(pig.status))
  ...
  async function removePig(pig:LitterPig){
    if(pig.herd_animal_id){setMessage('This pig is already in Herd Animals. Delete the herd-animal record first if it was entered in error.');return}
    if(!window.confirm(`Permanently delete pig #${pig.sequence_number} from this litter? This cannot be undone.`))return
    const{error}=await supabase.rpc('delete_litter_pig_permanently',{p_litter_pig_id:pig.id})
    setMessage(error?.message||`Pig #${pig.sequence_number} permanently deleted.`);if(!error)load()
  }
  async function archiveLitter(){
    if(current.archived_at){
      if(!window.confirm(`Return ${current.group_name} to the active Litters section?`))return
      const{error}=await supabase.from('offspring_groups').update({archived_at:null,updated_at:new Date().toISOString()}).eq('id',current.id)
      if(error)setMessage(error.message);else navigate('/litters',{replace:true})
      return
    }
    const incomplete=pigs.filter(p=>!p.herd_animal_id&&!completedStatuses.includes(p.status))
    if(incomplete.length){setMessage(`${incomplete.length} pig${incomplete.length===1?'':'s'} still need a completed status or must be moved to Herd Animals before this litter can be archived.`);return}
    if(!window.confirm(`Archive ${current.group_name}? It will move from active Litters to Archived Animals/Litters. Its records and exports will remain available.`))return
    const{error}=await supabase.rpc('archive_completed_litter',{p_offspring_group_id:current.id})
    if(error)setMessage(error.message);else navigate('/outside-herd',{replace:true})
  }
  async function deleteLitter(){
    if(!window.confirm(`Permanently delete ${current.group_name}? This is only for a litter entered in error. All litter-pig rows and the farrowing record will be erased. This cannot be undone.`))return
    if(!window.confirm(`Final confirmation: permanently erase the entire litter ${current.group_name}?`))return
    const{error}=await supabase.rpc('delete_litter_permanently',{p_offspring_group_id:current.id})
    if(error)setMessage(error.message);else navigate('/litters',{replace:true})
  }
  ...
  return <>
    ...
    <div className="page-head"><div><p className="eyebrow">LITTER & OFFSPRING RECORD</p><h1>{litter.group_name}</h1><p className="lead">{dam.call_name} × {litter.sire?.boar_name||'Sire not recorded'} • Farrowed {formatDate(litter.birth_date)}</p></div><span className="pill">{litter.archived_at?'Archived':litter.registry_association||'Unregistered'}</span></div>
    {message&&<p className="notice success-notice">{message}</p>}
    <div className="stat-grid"><Stat label="Total born" value={birth.total_born}/><Stat label="Born alive" value={birth.born_alive}/><Stat label="Stillborn" value={birth.stillborn}/><Stat label="Mummified" value={birth.mummified}/><Stat label="Total $ earned" value={currency(totalEarned)}/></div>
    ...
    <form className="panel form-grid" onSubmit={saveHeader}>
      ...
      <BreedSelect value={litter.breed||''} onChange={v=>setLitter({...litter,breed:v})}/><Input label="Litter ear notch" value={litter.litter_notch||''} onChange={v=>setLitter({...litter,litter_notch:v})}/><Input label="Litter number" value={litter.litter_number||''} onChange={v=>setLitter({...litter,litter_number:v})}/><Input label="Parity" type="number" min="1" value={litter.parity||''} onChange={v=>setLitter({...litter,parity:numberOrNull(v)})}/>
      ...
      <label className="full">Notes<textarea value={litter.notes||''} onChange={e=>setLitter({...litter,notes:e.target.value})}/></label><div className="form-actions split-actions full"><button type="button" className="button danger" onClick={deleteLitter}><Trash2/> Delete litter</button><div className="action-group">{(litter.archived_at||canArchive)&&<button type="button" className="button secondary" onClick={archiveLitter}><Archive/> {litter.archived_at?'Return to active litters':'Archive litter'}</button>}<button className="button primary"><Save/> Save litter details</button></div></div>
    </form>
    ...
    <section className="panel pig-section"><div className="section-heading"><div><h2>Animals in this litter</h2><p>These records are separate from Herd Animals.</p></div><button className="button secondary" onClick={addPig}><Plus/> Add pig</button></div>
      <div className="pig-list">{pigs.map(pig=><PigEditor key={pig.id} pig={pig} litterNotch={litter.litter_notch} onSaved={load} onMessage={setMessage} onSale={()=>setSalePig(pig)} onPromote={()=>promote(pig)} onDelete={()=>removePig(pig)}/>)}</div>
    </section>
    {salePig&&<SaleModal pig={salePig} close={()=>setSalePig(null)} saved={text=>{setSalePig(null);setMessage(text);load()}}/>}
  </>
}

function PigEditor({pig,litterNotch,onSaved,onMessage,onSale,onPromote,onDelete}:{pig:LitterPig;litterNotch:string|null;onSaved:()=>void;onMessage:(m:string)=>void;onSale:()=>void;onPromote:()=>void;onDelete:()=>void}){
  const [value,setValue]=useState(pig),[open,setOpen]=useState(false)
  async function save(){const terminal=['sold','culled','deceased','died','archived','stillborn','mummified'].includes(value.status);const{error}=await supabase.from('litter_pigs').update({pig_name:value.pig_name||null,sex_class:value.sex_class,individual_notch:value.individual_notch||null,registration_number:value.registration_number||null,registered_name:value.registered_name||null,teat_count_left:value.teat_count_left||null,teat_count_right:value.teat_count_right||null,birth_date:value.birth_date,birth_weight:value.birth_weight||null,weaning_date:value.weaning_date||null,weaning_weight:value.weaning_weight||null,status:value.status,status_date:terminal?(value.status_date||new Date().toISOString().slice(0,10)):value.status==='retained'?value.status_date:null,notes:value.notes||null,updated_at:new Date().toISOString()}).eq('id',value.id);onMessage(error?.message||`Pig #${value.sequence_number} saved.`);if(!error)onSaved()}
  return <article className="pig-editor"><button className="pig-summary" onClick={()=>setOpen(!open)}><span className="pig-number">#{pig.sequence_number}</span><span><strong>{value.pig_name||`Litter pig ${pig.sequence_number}`}</strong><small>{[value.sex_class,[litterNotch,value.individual_notch].filter(Boolean).join('-'),value.status].filter(Boolean).join(' • ')}</small></span><span className="pig-row-actions"><ShoppingCart onClick={e=>{e.stopPropagation();onSale()}}/><UserRoundPlus onClick={e=>{e.stopPropagation();onPromote()}}/><ExternalLink className={open?'rotate':''}/></span></button>{open&&<div className="pig-edit-grid"><Input label="Pig name" value={value.pig_name||''} onChange={v=>setValue({...value,pig_name:v})}/><label>Sex / class<select value={value.sex_class} onChange={e=>setValue({...value,sex_class:e.target.value})}><option value="unknown">Unknown</option><option value="boar">Boar</option><option value="gilt">Gilt</option><option value="barrow">Barrow</option></select></label><Input label="Individual notch" value={value.individual_notch||''} onChange={v=>setValue({...value,individual_notch:v})}/><Input label="Birth date" type="date" value={value.birth_date} onChange={v=>setValue({...value,birth_date:v})}/><Input label="Birth weight" type="number" min="0" step=".01" value={value.birth_weight||''} onChange={v=>setValue({...value,birth_weight:numberOrNull(v)})}/><Input label="Weaning date" type="date" value={value.weaning_date||''} onChange={v=>setValue({...value,weaning_date:v||null})}/><Input label="Weaning weight" type="number" min="0" step=".01" value={value.weaning_weight||''} onChange={v=>setValue({...value,weaning_weight:numberOrNull(v)})}/><Input label="Teats - left" type="number" min="0" value={value.teat_count_left||''} onChange={v=>setValue({...value,teat_count_left:numberOrNull(v)})}/><Input label="Teats - right" type="number" min="0" value={value.teat_count_right||''} onChange={v=>setValue({...value,teat_count_right:numberOrNull(v)})}/><label>Status<select value={value.status} onChange={e=>setValue({...value,status:e.target.value})}><option value="alive">Alive</option><option value="for_sale">For sale</option><option value="weaned">Weaned</option><option value="sold">Sold</option><option value="culled">Culled</option><option value="deceased">Deceased</option><option value="archived">Archived</option><option value="stillborn">Stillborn</option><option value="mummified">Mummified</option><option value="outcome_not_recorded">Outcome not recorded</option><option value="retained">Moved to Herd</option></select></label><Input label="Status date" type="date" value={value.status_date||''} onChange={v=>setValue({...value,status_date:v||null})}/><Input label="Registered name" value={value.registered_name||''} onChange={v=>setValue({...value,registered_name:v})}/><Input label="Registration number" value={value.registration_number||''} onChange={v=>setValue({...value,registration_number:v})}/><label className="full">Notes<textarea value={value.notes||''} onChange={e=>setValue({...value,notes:e.target.value})}/></label><div className="pig-buttons full"><button className="button danger" type="button" onClick={onDelete} disabled={!!value.herd_animal_id}><Trash2/> Delete pig</button><button className="button secondary" type="button" onClick={onSale}><ShoppingCart/> Sale info</button><button className="button secondary" type="button" onClick={onPromote} disabled={!!value.herd_animal_id}><UserRoundPlus/> {value.herd_animal_id?'Already in herd':'Move to Herd'}</button><button className="button primary" type="button" onClick={save}><Save/> Save pig</button></div></div>}</article>
}
...
function Stat({label,value}:{label:string;value:number|string}){return <div className="stat"><strong>{value}</strong><span>{label}</span></div>}
